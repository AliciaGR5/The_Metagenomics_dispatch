...
for (newfile in files) {
  ...
  family_v <- rep(NA, nrow(f)) # Vector where the selected rank will be save. 
  ...
  f <- cbind(f, family_v) # Add as a new column
  
  # Get the diversity: 
  mym <- t(table(f$family_v))  #  Columns are the taxo names ...
...
write.table(mymergedt, file = "Diversity_table_family.txt",sep = "\t")

...
# We can use the table generated with "Diversity_table_g_AGR.R" or "Diversity_table_f_AGR.R" script
...
readfile <- read.csv("Diversity_table_family.txt",header=T,row.names=1,sep="\t") # Reading "diversity_table" file
...
write.table(my_df, file = "Shannon_table_family.txt",sep = "\t")
...
ggplot(my_df, aes(x=Samples, y=AD_Shannon)) +
  ...
  theme(
    ...
    panel.grid.major.x = element_line(size = 0.5, linetype =2, # Longest lines that go through the plot
                                      colour = "grey90"),
    #panel.background = element_blank(), # Another way to have white background
    ...
    legend.position = c(0.86, 0.3),        # Change legend position
  ...
  xlab("Samples (Family)")                 # Renaming x axis


## SAVE ##

ggsave("Job1_alphadiversity_f.png", width = 30, height = 25, units = "cm", bg = "transparent")
ggsave("Job1_alphadiversity_f.pdf", width = 30, height = 25, units = "cm", bg = "transparent")
